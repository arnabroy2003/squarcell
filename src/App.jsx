...
const SquarcellLanding = () => {
  ...
  return (
    <div className="relative min-h-screen bg-[#06080D] text-white overflow-hidden selection:bg-purple-500/30">
      ...
      {/* 🧭 NAVBAR */}
      <nav className="fixed top-0 w-full z-[100] px-10 py-6 flex justify-between items-center backdrop-blur-md border-b border-white/5">
        <div className="flex items-center gap-3 group cursor-pointer">
  {/* Logo Container */}
  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center group-hover:rotate-[10deg] transition-transform duration-500 shadow-[0_0_20px_rgba(255,255,255,0.3)] overflow-hidden">
    {/* Image Tag */}
    <img 
      src="/logo/sr.png" // Apnar logo-r path ekhane din
      alt="Squarcell Logo"
      className="w-full h-full object-contain p-1.5" // p-1.5 padding ektu gap rakhbe edges theke
    />
  </div>
  
  <span className="text-lg font-bold tracking-tighter uppercase">Squarcell Group</span>
</div>
        ...
        >
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight leading-[1] lg:leading-[0.95] mb-5 lg:mb-6">
            {hoveredCard 
              ...
              : <>Creating innovation across <span className="text-gray-500">education & infra.</span></>
            }
          </h1>
  ...
  <div className="w-full lg:w-[55%] flex flex-col lg:flex-row gap-4 h-auto lg:h-[550px]">
    ...
    {ventures.map((v) => (
      <motion.div
        ...
        className="relative rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden cursor-pointer bg-[#0D111A] border border-white/10 group min-h-[220px] sm:min-h-[260px]"
        animate={{ 
      ...
      >
        ...
        <div className="absolute -top-6 lg:-top-10 -right-2 lg:-right-5 text-[7rem] sm:text-[10rem] lg:text-[15rem] font-black text-white/[0.03] group-hover:text-white/[0.05] transition-colors pointer-events-none select-none">
          {v.id}
        </div>
          ...
          <AnimatePresence mode="wait">
            {hoveredCard === v.id && (
              ...
              >
                ...
                >
                  <div className="w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 relative">
                    <img 
                      src={v.logo} 
                      alt={v.name} 
